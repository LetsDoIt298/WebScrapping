...
from flask_cors import CORS, cross_origin #Required for deployment in Asure,GCP,Heroku,AWS......This prevents IP's from getting blocked in terms of cross connection
import pandas as pd
#own
from mongoDBOperations import MongoDBManagement
#own
from FlipkratScrapping import FlipkratScrapper
from selenium import webdriver #webdriver for chrome to scrape
...
free_status = True #kind of Flag(variable with BOOLEAN value)
...
@cross_origin() #To ignoore error -> if deployment is done on cloud, and if region is different then we'll get error
def index():
    if request.method == 'POST':
        ...
        try:
            review_count = 0
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(), #object of FlipkratScrapper
